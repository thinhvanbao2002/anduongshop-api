...
const searchProduct = async ({ perPage, keyword, page }) => {
  try {
    // Tạo điều kiện tìm kiếm với $regex cho MongoDB
    const getKeyword = {
      name: { $regex: keyword, $options: "i" }, // Tìm kiếm tương đối không phân biệt chữ hoa chữ thường
    };

    // Đếm số lượng tài liệu phù hợp
    const count = await ProductModel.countDocuments(getKeyword);

    // Tìm tất cả tài liệu phù hợp với điều kiện và phân trang
    const data = await ProductModel.find(getKeyword)
      .limit(perPage)
      .skip((page - 1) * perPage);
...
const updateProductAvailable = async ({ idProduct, amount }) => {
  ...
  existingProduct.productsAvailable =
    existingProduct.productsAvailable - amount;
...
const deleteProduct = async (idProduct) => {
  ...
  if (checkOrder) {
    throw new Error(
      "Cannot delete product because there are order associated with it."
    );
  }

  const deletedProduct = await ProductModel.findByIdAndDelete(idProduct);
  const deletedDetailImageProduct = await DetailImageModel.findByIdAndDelete(
    idProduct
  );
  if (!deletedProduct) {
...
const exportExcel = async () => {
  try {
    const dataOProducts = await ProductModel.find().populate("idCategory");
