...
const login = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ data: response }, process.env.JWT_SECRET, {
      expiresIn: "30 days",
    });
...
const signin = async (req, res) => {
  try {
    ...
    // const subject = "Hello ✔";
    // const userToken = jwt.sign({ data: email }, process.env.JWT_SECRET, {
    //   expiresIn: "3m",
    // });
    ...
    form.parse(req, async (err, fields, files) => {
      try {
        ...
        if (fileMimeType.startsWith("image/")) {
          const response = await userService.updateAvtUser({
            userID,
            filePath,
          });
          return res.status(200).json({
...
const verifyEmail = async (req, res) => {
  try {
    ...
    const errorMessage =
      '<div style="text-align: center; padding: 20px; background-color: #f2dede; color: #a94442; border: 1px solid #ebccd1; border-radius: 5px;"><h1>Email verification failed</h1></div>';
